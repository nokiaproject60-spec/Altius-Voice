...
/**
 * Removes regulatory citations like:
 * [Page 12, Quote: "primary efficacy endpoint was met"]
 */
export function stripCitationsForSpeech(text: string): string {
  if (!text) return text;

  return text
    // Remove [Page X, Quote: "..."]
    .replace(/\[Page\s+\d+,[^\]]+\]/gi, '')
    // Remove simple [Page X]
    .replace(/\[Page\s+\d+\]/gi, '')
    // Remove (Page X)
    .replace(/\(Page\s+\d+\)/gi, '')
    // Clean extra spaces
